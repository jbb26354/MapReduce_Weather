...
public class TotalRainfall extends Configured implements Tool 
{
    ...
    public void map(LongWritable inputKey, Text inputValue, Context context) throws IOException, InterruptedException
    {
      ...
      for(int i = 0; i < element.length; i++)
      {
        if(st.hasMoreTokens())
        {
          ...
          if(i == 0 && element[i].equals("StationID")) return;
        }
        else return;
      } 
          
      context.write(new Text(element[ZIPCODE] + " " + element[YEAR]), new DoubleWritable(Double.parseDouble(element[TEMP])));
  ...
  public int run(String[] args) throws Exception 
  {
    if(args.length != 3)
    {
      System.out.println("Usage: [path]hadoop jar <jarname> <jarclass> <directoryName of Input Data> <ouput directory> <number of reducers>");
    ...
    try
    {
      numReducers = new Integer(args[2]);
      System.out.println("number reducers set to: " + numReducers);
    }
    catch(NumberFormatException e)
    {
      System.out.println("Usage: [path]hadoop jar <jarname> <jarclass> <directoryName of Input Data> <ouput directory> <number of reducers>");
      System.out.println("Error: number of reducers not a type integer");
      return -1;
    }
    ...
    Job job = new Job(conf, "Project 1 Part 1");
